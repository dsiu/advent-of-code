...
function setVisibilityForest(forest) {
  ...
  let f = Utils$AdventOfCode.compose(setVisibilityOrient, rotate);
...
let part1 = Utils$AdventOfCode.compose(setVisibilityForest, countVisible);
...
function part2(forest) {
  ...
  return Utils$AdventOfCode.maxIntInArray(Stdlib__Array.combination2(Stdlib__Array.makeBy(nrows - 1 | 0, id), Stdlib__Array.makeBy(ncols - 1 | 0, id), (r, c) => scenicScore(forest, r, c)));
}

function parse(data) {
  return Utils$AdventOfCode.splitNewline(data).map(x => Utils$AdventOfCode.splitChars(x.trim()).map(x => ({
    TAG: "Tree",
