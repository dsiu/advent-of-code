// Generated by ReScript, PLEASE EDIT WITH CARE

import * as Curry from "rescript/lib/es6/curry.js";

function cmp(a, b) {
  return a - b | 0;
}

function add(a, b) {
  return a + b | 0;
}

var IntElem = {
  cmp: cmp,
  add: add
};

function cmp$1(a, b) {
  return a - b | 0;
}

function add$1(a, b) {
  return a + b;
}

var FloatElem = {
  cmp: cmp$1,
  add: add$1
};

function Make_V2(E) {
  var make = function (a, b) {
    return [
            a,
            b
          ];
  };
  var cmp = function (param, param$1) {
    var r = Curry._2(E.cmp, param[0], param$1[0]);
    if (r !== 0) {
      return r;
    } else {
      return Curry._2(E.cmp, param[1], param$1[1]);
    }
  };
  var add = function (param, param$1) {
    return [
            Curry._2(E.add, param[0], param$1[0]),
            Curry._2(E.add, param[1], param$1[1])
          ];
  };
  return {
          cmp: cmp,
          add: add,
          make: make
        };
}

function Make_V3(E) {
  var make = function (a, b, c) {
    return [
            a,
            b,
            c
          ];
  };
  var cmp = function (param, param$1) {
    var r = Curry._2(E.cmp, param[0], param$1[0]);
    if (r !== 0) {
      return r;
    }
    var r$1 = Curry._2(E.cmp, param[1], param$1[1]);
    if (r$1 !== 0) {
      return r$1;
    } else {
      return Curry._2(E.cmp, param[2], param$1[2]);
    }
  };
  var add = function (param, param$1) {
    return [
            Curry._2(E.add, param[0], param$1[0]),
            Curry._2(E.add, param[1], param$1[1]),
            Curry._2(E.add, param[2], param$1[2])
          ];
  };
  return {
          cmp: cmp,
          add: add,
          make: make
        };
}

function make(a, b) {
  return [
          a,
          b
        ];
}

function cmp$2(param, param$1) {
  var r = param[0] - param$1[0] | 0;
  if (r !== 0) {
    return r;
  } else {
    return param[1] - param$1[1] | 0;
  }
}

function add$2(param, param$1) {
  return [
          param[0] + param$1[0] | 0,
          param[1] + param$1[1] | 0
        ];
}

var V2_Int = {
  cmp: cmp$2,
  add: add$2,
  make: make
};

function make$1(a, b) {
  return [
          a,
          b
        ];
}

function cmp$3(param, param$1) {
  var r = param[0] - param$1[0] | 0;
  if (r !== 0) {
    return r;
  } else {
    return param[1] - param$1[1] | 0;
  }
}

function add$3(param, param$1) {
  return [
          param[0] + param$1[0],
          param[1] + param$1[1]
        ];
}

var V2_Float = {
  cmp: cmp$3,
  add: add$3,
  make: make$1
};

function make$2(a, b, c) {
  return [
          a,
          b,
          c
        ];
}

function cmp$4(param, param$1) {
  var r = param[0] - param$1[0] | 0;
  if (r !== 0) {
    return r;
  }
  var r$1 = param[1] - param$1[1] | 0;
  if (r$1 !== 0) {
    return r$1;
  } else {
    return param[2] - param$1[2] | 0;
  }
}

function add$4(param, param$1) {
  return [
          param[0] + param$1[0] | 0,
          param[1] + param$1[1] | 0,
          param[2] + param$1[2] | 0
        ];
}

var V3_Int = {
  cmp: cmp$4,
  add: add$4,
  make: make$2
};

export {
  IntElem ,
  FloatElem ,
  Make_V2 ,
  Make_V3 ,
  V2_Int ,
  V2_Float ,
  V3_Int ,
}
/* No side effect */
