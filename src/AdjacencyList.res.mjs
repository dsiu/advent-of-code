...
function fromArray2D(array2D) {
  let addEdges = (adjList, x, y) => {
    ...
    neighbors.filter(param => Array2D$AdventOfCode.isValidXY(array2D, [
      param[0],
      param[1]
    ])).forEach(param => {
      let neighborNode_0 = param[0];
      let neighborNode_1 = param[1];
      let neighborNode = [
        neighborNode_0,
        neighborNode_1
      ];
      addNode$4(adjList, neighborNode);
      addDirectedEdge$4(adjList, currentNode, neighborNode, undefined);
    });
  };
  return Array2D$AdventOfCode.reduceWithIndex(array2D, make(), (acc, param, param$1) => {
    addEdges(acc, param$1[0], param$1[1]);
    return acc;
  });
}
