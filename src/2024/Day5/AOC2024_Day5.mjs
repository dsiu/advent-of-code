...
function reorder(rules, _printed, unprinted) {
  while (true) {
    ...
    let next = Utils$AdventOfCode.minIntInArray(Stdlib__Set.toArray(candidate));
...
function solvePart1(data) {
  ...
  return Utils$AdventOfCode.sumIntArray(match[1].filter(__x => isOrderValid(__x, rules)).map(middlePage));
}

function solvePart2(data) {
  let match = parse(data);
  let rules = makeRules(match[0]);
  return Utils$AdventOfCode.sumIntArray(match[1].filter(x => !isOrderValid(x, rules)).map(p => reorder(rules, [], new Set(p))).map(middlePage));
}
