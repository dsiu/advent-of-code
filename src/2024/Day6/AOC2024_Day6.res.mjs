...
function bounds(grid) {
  ...
  return [
    [
      Utils.minIntInArray(rows),
      Utils.minIntInArray(cols)
    ],
    [
      Utils.maxIntInArray(rows),
      Utils.maxIntInArray(cols)
    ]
  ];
}
...
function toString(grid) {
  ...
  let rows = Stdlib__Array.fromInitializer((match[0] - minR | 0) + 1 | 0, i => {
    let cols = Stdlib__Array.fromInitializer((maxC - minC | 0) + 1 | 0, j => {
      ...
      return String(Stdlib__Option.getExn(grid.get(Coord_V2.toString(c)), undefined));
    });
    return cols.join("");
  });
  return rows.join("\n");
...
function walkAble(grid, c) {
  let y = grid.get(Coord_V2.toString(c));
  if (y !== undefined) {
    return y !== "#";
  } else {
    return true;
...
function guardEq(a, b) {
  if (Coord_V2.compare(a.pos, b.pos) === 0) {
...
function step(param, guard) {
  ...
  let ahead = Coord_V2.add(guard.pos, guard.dir._0);
...
function parse(data) {
  return Utils.splitNewline(data).map(l => l.trim().split("").map(fromString));
}

function init(data) {
  let m = fromArray(parse(data));
  let guard_pos = Stdlib__Option.getExn(Stdlib__Option.flatMap(Stdlib__Option.map(find(m.grid, (_k, v) => v === "^"), param => param[0]), Coord_V2.fromString), undefined);
  let guard = {
    pos: guard_pos,
    dir: up
  };
...
function solvePart2(data) {
  ...
  let grid = m.grid;
  let guard = match[1];
  let news = Stdlib__Array.uniq(walk(m, guard)).filter(x => Coord_V2.compare(x, guard.pos) !== 0);
  return news.filter(g => {
    let m$p = clone(grid);
    m$p.set(Coord_V2.toString(g), "#");
    return isLoop(guard, [], {
      grid: m$p,
      bounds: bounds
    });
