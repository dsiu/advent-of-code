...
function parse(data) {
  return Belt_Array.map(data.split(","), Utils.intFromStringExn);
}
...
let InvalidOpCode = /* @__PURE__ */Primitive_exceptions.create("AOC2019_Day2.IntCode.InvalidOpCode");
...
function getInstr(t) {
  let op = Belt_Array.getExn(t.prog, t.pc);
  let arg1 = Belt_Array.getExn(t.prog, t.pc + 1 | 0);
  let arg2 = Belt_Array.getExn(t.prog, t.pc + 2 | 0);
  let out = Belt_Array.getExn(t.prog, t.pc + 3 | 0);
  if (op >= 3) {
    if (op === 99) {
      return;
    }
...
function executeWithNounVerb(prog, noun, verb) {
  let result = execute(setNounVerb(prog, noun, verb));
...
function part2(xs) {
  return Belt_Array.getExn(Stdlib__Array.combinationIf2(Belt_Array.range(0, 99), Belt_Array.range(0, 99), (noun, verb) => {
