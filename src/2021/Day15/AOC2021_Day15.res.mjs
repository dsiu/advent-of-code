...
function makeNodes(g, lines) {
  return Array2D.mapWithIndex(lines, (c, param) => {
...
function make(lines) {
  ...
  let g$1 = Array2D.reduceWithIndex(lines, g, (g, _e, c) => {
...
function solve(param) {
  ...
  let dest_x = Array2D.lengthX(nodes) - 1 | 0;
  let dest_y = Array2D.lengthY(nodes) - 1 | 0;
...
function parse(data) {
  return Belt_Array.map(Utils.splitNewline(data), l => Belt_Array.map(Utils.splitChars(l.trim()), Utils.intFromStringExn));
}
