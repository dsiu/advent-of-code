...
function countMark(p) {
  return Array2D$AdventOfCode.reduce(p, 0, (a, mark) => {
    if (mark === "#") {
      return a + 1 | 0;
    } else {
      return a;
...
function makePaper(coords) {
  ...
  let p = Array2D$AdventOfCode.make(sizes, ".");
  Belt_Array.forEach(coords, c => Array2D$AdventOfCode.set(p, c, "#"));
  return p;
}
...
function getTransformedSize(t, param) {
  let n = param[1];
  let sizeX = Array2D$AdventOfCode.lengthX(t);
  let sizeY = Array2D$AdventOfCode.lengthY(t);
  if (param[0] === "Y") {
    return [
      sizeX,
      n
...
function transform(t, f) {
  ...
  let t$p = Array2D$AdventOfCode.make(new_size, ".");
  return Array2D$AdventOfCode.reduceWithIndex(t, t$p, (a, mark, coord) => {
    let coord$p = transformCoord(coord, f);
    let new_mark = Array2D$AdventOfCode.get(t, coord$p);
...
function parse(data) {
  ...
  return [
    Belt_Array.map(Utils$AdventOfCode.splitNewline(coords), x => {
      let s = x.trim().split(",");
      return [
    ...
    Belt_Array.map(Utils$AdventOfCode.splitNewline(folds), x => {
