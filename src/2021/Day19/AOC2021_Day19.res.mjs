...
import * as Linear from "../../Linear.res.mjs";
import * as Bag$Bag from "@dsiu/rescript-bag/src/bag.res.mjs";
...
function toString(t) {
  return Utils.Printable.$$Array.toString(t, param => "scannerName: " + String(param.scannerName) + ", beacons: " + Utils.Printable.$$Array.toString(param.beacons, param => {
    let match = param._0;
    return "(" + String(match[0]) + ", " + String(match[1]) + ", " + String(match[2]) + ")";
...
function cmp$2(a, b) {
  return Linear.V3.cmp(a._0, b._0);
}
...
function parse(data) {
  ...
  let parseOne = data => {
    let lines = Belt_Array.map(Utils.splitNewline(data), prim => prim.trim());
    let name = Utils.intFromStringExn(Belt_Array.getExn(lines, 0).replace("--- scanner ", "").replace(" ---", ""));
  ...
  return Belt_Array.map(Utils.splitDoubleNewline(data), parseOne);
}
