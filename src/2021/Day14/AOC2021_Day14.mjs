...
function update_value_inc_by_n(h, k, n) {
  return Utils$AdventOfCode.mutableMapStringUpdate(h, k, __x => Utils$AdventOfCode.increaseByBigInt(__x, n));
}
...
function solve_with_result(t, n) {
  ...
  let r = Belt_MutableMapString.toArray(Belt_List.reduce(ret, Belt_MutableMapString.make(), (acc, k) => Utils$AdventOfCode.mutableMapStringUpdate(acc, k, Utils$AdventOfCode.increaseBy1L)));
  let match = Utils$AdventOfCode.maxKeyBigIntValuePair(r);
  let match$1 = Utils$AdventOfCode.minKeyBigIntValuePair(r);
...
function countPolymers(m, template) {
  let r = Belt_MutableMapString.make();
  Belt_MutableMapString.forEach(m, (k, v) => Belt_Array.forEach(Utils$AdventOfCode.splitChars(k), c => {
...
function solve(t, n) {
  ...
  let match = Utils$AdventOfCode.maxKeyBigIntValuePair(c);
  let match$1 = Utils$AdventOfCode.minKeyBigIntValuePair(c);
  return match[1] - match$1[1];
}
...
function parse(data) {
  let parsed = Utils$AdventOfCode.splitDoubleNewline(data);
  let template = Belt_Option.getExn(Belt_Array.get(parsed, 0));
  let rules = Belt_Option.getExn(Belt_Array.get(parsed, 1));
  return [
    template.trim().split(""),
    Belt_Array.map(Utils$AdventOfCode.splitNewline(rules), x => {
      let s = x.trim().split(" -> ");
      return [
