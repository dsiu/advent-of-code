...
function increaseEnergy(t) {
  return Array2D.map(t, x => Utils.add(1, x));
}

function getFlashingCoords(t) {
  return Array2D.reduceWithIndex(t, [], (a, e, coord) => {
    if (e > 9) {
...
function performFlash(t, coord) {
  ...
  Belt_Array.forEach(neighbors, n_addr => {
    ...
    Array2D.set(t, n_addr, orig > 0 ? orig + 1 | 0 : orig);
  });
  return t;
}

function dim(t, coord) {
  Array2D.set(t, coord, 0);
}
...
function iterate(t) {
  ...
  while (true) {
    ...
    Belt_Array.forEach(flashings, flash_coord => {
      Array2D.set(performFlash(next, flash_coord), flash_coord, 0);
    });
    continue;
  };
}
...
function countFlashN(t, n) {
  return iterateAndReduceN(t, n, 0, (acc, t) => acc + countZero(Array2D.flatten(t)) | 0);
}
...
function flashesAtN(t, n) {
  return iterateAndReduceN(t, n, 0, (param, t) => countZero(Array2D.flatten(t)));
}

function toString(t) {
  let ret = [];
  for (let i = 0, i_finish = Array2D.lengthY(t); i < i_finish; ++i) {
    let row = Belt_Option.getWithDefault(Array2D.getYEquals(t, i), []);
