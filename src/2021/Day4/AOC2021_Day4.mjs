...
function toString(t) {
  return Array2D.toString(t, __x => __x.toString());
}
...
function solve(t, match_draws) {
  ...
  let matchY = helper(t, 0, Array2D.getYEquals, Array2D.lengthY(t));
  let matchX = helper(t, 0, Array2D.getXEquals, Array2D.lengthX(t));
  let remove_marked = (t, m) => Belt_SetInt.toArray(Belt_SetInt.diff(Belt_SetInt.fromArray(Array2D.flatten(t)), Belt_SetInt.fromArray(m)));
...
function make$2(lines) {
  return Belt_Array.map(lines, b => make$1(Belt_Array.map(Utils.splitNewline(b), prim => prim.trim())));
}

function toString$1(t) {
  return Utils.Printable.$$Array.toString(t, toString);
}
...
function parse(data) {
  let lines = Belt_Array.map(Utils.splitDoubleNewline(data), prim => prim.trim());
