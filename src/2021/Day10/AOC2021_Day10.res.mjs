...
import * as Stack_Array$AdventOfCode from "../../Stack_Array.res.mjs";

function log(prim) {
  console.log(prim);
}

let ParseError = /* @__PURE__ */Primitive_exceptions.create("AOC2021_Day10-AdventOfCode.ParseError");

let NotSupported = /* @__PURE__ */Primitive_exceptions.create("AOC2021_Day10-AdventOfCode.NotSupported");
...
function makeParseTree(xs) {
  ...
  while (true) {
    ...
    let last = Stack_Array$AdventOfCode.peek(stack);
    let match$2 = isCloseBracket($$this._0);
    if (match$2) {
      if (last !== undefined) {
        let match$3 = Stack_Array$AdventOfCode.pop(stack);
        _stack = match$3[1];
        _tree = add(tree, {
          TAG: "Node",
          l: last,
...
function parse(data) {
  return Belt_Array.map(Utils$AdventOfCode.splitNewline(data), extra => Stdlib__Function.compose(prim => prim.trim(), Utils$AdventOfCode.splitChars, extra));
}
