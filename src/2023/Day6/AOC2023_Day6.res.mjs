...
function parse(data) {
  let lines = Utils$AdventOfCode.splitNewline(data).map(prim => prim.trim());
  let parseLine = (line, prefix) => Stdlib__Option.getExn(Stdlib__Option.map(line, s => Utils$AdventOfCode.splitSpace(s.replace(prefix, "")).filter(extra => Stdlib__Function.complement(Stdlib__String.isEmpty, extra)).map(Utils$AdventOfCode.intFromStringExn)), undefined);
