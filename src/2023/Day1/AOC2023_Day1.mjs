...
function spelledOutToDigits(str, matchDir) {
  ...
  return Belt_Array.reduce(Utils$AdventOfCode.splitChars(tmp), "", (acc, c) => {
    let tmp;
...
function get2Digits(str) {
  let digits = Stdlib__Array.filterMap(Utils$AdventOfCode.splitChars(str), none => Stdlib__Int.fromString(none, 10));
...
function part2(xs) {
  return Utils$AdventOfCode.sumIntArray(xs.map(x => {
...
function part1(xs) {
  return Utils$AdventOfCode.sumIntArray(xs.map(Utils$AdventOfCode.compose(get2Digits, combineFirstAndLast)));
}
