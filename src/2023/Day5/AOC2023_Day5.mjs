...
function runWithInterval(t, src) {
  ...
  let newSrc = Stdlib__Option.getOr(Stdlib__Option.map(intersection, i => Interval$AdventOfCode.remove(src, i)), src);
  let newDest = Stdlib__Option.map(intersection, i => Interval$AdventOfCode.add(i, t.offset));
  return [
    newSrc,
    newDest
  ];
...
function parse(data) {
  ...
  let parseSeed = line => Utils$AdventOfCode.splitSpace(Stdlib__Option.getExn(line.split(": ")[1], undefined)).map(prim => BigInt(prim));
  let parseMap = lines => {
    let categoryLine = Stdlib__Option.getExn(lines[0], undefined);
    let srcDestLines = lines.slice(1);
    let match = Stdlib__Option.getExn(Stdlib__Option.flatMap(Utils$AdventOfCode.splitSpace(categoryLine)[0], s => s.split("-to-")), undefined);
    if (match.length !== 2) {
      throw {
