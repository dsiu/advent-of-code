...
import * as Interval$AdventOfCode from "../../Interval.res.mjs";
...
function runWithInterval(t, src) {
  let intersection = Interval$AdventOfCode.intersect(src, t.srcInterval);
  let newSrc = Stdlib_Option.getOr(Stdlib_Option.map(intersection, i => Interval$AdventOfCode.remove(src, i)), src);
  let newDest = Stdlib_Option.map(intersection, i => Interval$AdventOfCode.add(i, t.offset));
  return [
    newSrc,
    newDest
  ];
...
function parse(data) {
  let lines = Utils$AdventOfCode.splitDoubleNewline(data).map(l => Utils$AdventOfCode.splitNewline(l).map(prim => prim.trim()));
  let seedLine = Stdlib_Option.getExn(Stdlib_Option.flatMap(lines[0], __x => __x[0]), undefined);
  let mapLines = lines.slice(1);
  let parseSeed = line => Utils$AdventOfCode.splitSpace(Stdlib_Option.getExn(line.split(": ")[1], undefined)).map(prim => BigInt(prim));
  let parseMap = lines => {
    let categoryLine = Stdlib_Option.getExn(lines[0], undefined);
    let srcDestLines = lines.slice(1);
    let match = Stdlib_Option.getExn(Stdlib_Option.flatMap(Utils$AdventOfCode.splitSpace(categoryLine)[0], s => s.split("-to-")), undefined);
    if (match.length !== 2) {
      throw {
    ...
    let parseIntervalLine = l => {
      let match = Utils$AdventOfCode.splitSpace(l).map(prim => BigInt(prim));
      if (match.length !== 3) {
        throw {
          RE_EXN_ID: "Match_failure",
          _1: [
...
function findLocation(almanac, seedTransform) {
  ...
  return Utils$AdventOfCode.minBigIntInArray(locations.map(param => param[0]));
}

function makeSeedsInterval(seeds) {
  return seeds.map(__x => Interval$AdventOfCode.makeWithLength(__x, BigInt(1)));
}
