...
function parse(data) {
  return Utils.splitNewline(data).map(l => {
    let ll = l.trim().split(": ");
    let id = Stdlib_Option.flatMap(ll.at(0), Utils.compose(__x => __x.replace("Game ", ""), __x => Stdlib_Int.fromString(__x, 10)));
    let draws = ll.at(1).split("; ").map(eachDraw => eachDraw.split(", ").map(singleDraw => {
      let d = singleDraw.split(" ");
      let nColor = Stdlib_Option.flatMap(d.at(0), none => Stdlib_Int.fromString(none, 10));
      let color = d.at(1);
...
function maxNumColorsEachGame(draws) {
  return Stdlib_Array.reduce(draws, [
    {
      TAG: "Red",
      _0: 0
    },
...
function maxColorWithLimits(games, colorLimits) {
  return Stdlib_Array.filterMap(games, param => {
...
function part1(games) {
  return Utils.sumIntArray(maxColorWithLimits(games, [
    {
      TAG: "Red",
      _0: 12
    },
...
function part2(games) {
  let maxInt = Stdlib_Int.Constants.maxValue;
  let colorLimits_0 = {
    TAG: "Red",
    _0: maxInt
  };
  ...
  return Utils.sumIntArray(maxColorWithLimits(games, colorLimits).map(param => {
    let match = param[1];
    let r = match[0];
    switch (r.TAG) {
      case "Red" :
