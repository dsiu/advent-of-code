...
import * as ChineseRemainder$AdventOfCode from "../../ChineseRemainder.res.mjs";
...
function parse(data) {
  let lines = Belt_Array.map(Utils$AdventOfCode.splitNewline(data), x => x.trim());
  let timeStamp = Utils$AdventOfCode.intFromStringExn(Belt_Option.getExn(Belt_Array.get(lines, 0)));
  let isNotX = c => c !== "x";
  let bus_numbers = Belt_Array.map(Belt_Array.keep(Belt_Option.getExn(Belt_Array.get(lines, 1)).split(","), isNotX), Utils$AdventOfCode.intFromStringExn);
  return [
    timeStamp,
    bus_numbers
  ];
