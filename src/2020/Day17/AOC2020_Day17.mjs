...
function compare(a, b) {
  if (a.TAG === "Coord_V3") {
    if (b.TAG === "Coord_V3") {
      ...
      return PervasivesU.failwith("Invalid comparison");
    }
...
function add(a, b) {
  if (a.TAG === "Coord_V3") {
    if (b.TAG === "Coord_V3") {
      ...
      return PervasivesU.failwith("Invalid addition");
    }
  } else if (b.TAG === "Coord_V3") {
    return PervasivesU.failwith("Invalid addition");
  } else {
...
function update(grid) {
  ...
  return TableclothSet.union(TableclothSet.filter(grid, (function (__x) {
                    return cubeSurvives(grid, __x);
                  })), TableclothSet.filter(empties, (function (__x) {
                    return cubeBorn(grid, __x);
                  })));
}
...
function parse(data) {
  return Belt_Array.map(Utils$AdventOfCode.splitNewline(data), Utils$AdventOfCode.compose((function (prim) {
                    return prim.trim();
...
function solvePart1(data) {
  ...
  console.log(grid0);
  var finalGrid = iterate(grid0, update, 6);
  return TableclothSet.length(finalGrid);
}

function solvePart2(data) {
  var grid = conv34(makeGrid(parse(data)));
  var finalGrid = iterate(grid, update, 6);
  return TableclothSet.length(finalGrid);
}
