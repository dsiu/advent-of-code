...
function getFieldSafeAndValidate(t, field, f) {
  ...
  if (v !== undefined) {
    return f(Caml_option.valFromOption(v));
  } else {
    return false;
  }
}
...
function hgtValidator(s) {
  var x = (function (__x) {
        return Caml_option.null_to_opt(hgtRe.exec(__x));
      })(s);
  if (x === undefined) {
    return false;
  }
  var x$1 = Caml_option.valFromOption(x);
  var h = Belt_Option.getExn(Caml_option.nullable_to_opt(Belt_Array.getExn(x$1, 1)));
  var u = Belt_Option.getExn(Caml_option.nullable_to_opt(Belt_Array.getExn(x$1, 2)));
