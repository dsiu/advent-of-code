...
let InvalidStatus = /* @__PURE__ */Primitive_exceptions.create("AOC2020_Day12-AdventOfCode.InvalidStatus");
...
function parse(data) {
  return Belt_Array.map(Utils$AdventOfCode.splitNewline(data), x => {
    let s = x.trim();
    let code = s.charAt(0);
    let n = Utils$AdventOfCode.intFromStringExn(s.substring(1));
    return [
      code,
      n
    ];
...
function solve(data, algo) {
  ...
  ], prim => Math.abs(prim)), 0, Utils$AdventOfCode.add);
}
