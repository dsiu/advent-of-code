...
function ticketErrorRate(rules, tickets) {
  return Utils$AdventOfCode.sumIntArray(tickets.map(t => t.filter(v => !validForAnyField(rules, v))).flat());
}
...
function parse(data) {
  ...
  let match = Utils$AdventOfCode.splitDoubleNewline(data).map(x => Utils$AdventOfCode.splitNewline(x).map(trim));
  if (match.length !== 3) {
    throw {
  ...
  let nearby = match[2];
  let parseRange = s => {
    let match = s.split("-").map(Utils$AdventOfCode.intFromStringExn);
    if (match.length !== 2) {
      throw {
        RE_EXN_ID: "Match_failure",
        _1: [
  ...
  let parseTicket = s => s.split(",").map(Utils$AdventOfCode.intFromStringExn);
