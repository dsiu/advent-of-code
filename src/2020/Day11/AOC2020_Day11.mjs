...
let InvalidStatus = /* @__PURE__ */Primitive_exceptions.create("AOC2020_Day11-AdventOfCode.SeatMap.InvalidStatus");
...
function iteratePart1(t) {
  return Array2D$AdventOfCode.mapWithIndex(t, (param, s) => transformPart1(s, getAdjacents(t, [
    param[0],
    param[1]
  ])));
}
...
function nextSeatIn(t, _param, step) {
  while (true) {
    ...
    let seat = Stdlib__Option.getExn(Array2D$AdventOfCode.get(t, c), undefined);
    if (seat !== ".") {
      return seat;
    }
    _param = c;
...
function iteratePart2(t) {
  return Array2D$AdventOfCode.mapWithIndex(t, (param, s) => transformPart2(s, getDirectionals(t, [
    param[0],
    param[1]
  ])));
}
...
function make$1(xs) {
  ...
  xs.forEach((ys, y) => {
    Utils$AdventOfCode.splitChars(ys).forEach((c, x) => {
...
function toString(t) {
  let s = "";
  for (let y = 0, y_finish = Array2D$AdventOfCode.lengthY(t); y < y_finish; ++y) {
