...
function Dijkstra(A) {
  return NodeMap => {
    let dijkstra = (graph, startNode) => {
      ...
      let pq = PriorityQueue$AdventOfCode.MinPriorityQueue.push(PriorityQueue$AdventOfCode.MinPriorityQueue.empty, 0, startNode);
      let loop = (dist, prev, pq) => {
        let match = PriorityQueue$AdventOfCode.MinPriorityQueue.pop(pq);
        if (match === undefined) {
          return;
        }
        ...
        A.neighbors(graph, u).forEach(v => {
          ...
          if (match !== undefined) {
            ...
            if (dist_v !== undefined && alt < dist_v) {
              ...
              return loop(dist, prev, PriorityQueue$AdventOfCode.MinPriorityQueue.push(newQueue, alt, v));
            } else {
              return;
            }
          }
          NodeMap.set(dist, v, alt);
          NodeMap.set(prev, v, Primitive_option.some(u));
          loop(dist, prev, PriorityQueue$AdventOfCode.MinPriorityQueue.push(newQueue, alt, v));
        });
      };
      loop(dist, prev, pq);
      return [
