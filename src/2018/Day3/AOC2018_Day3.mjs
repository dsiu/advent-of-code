...
function parseLine(s) {
  var x = (function (__x) {
        return Caml_option.null_to_opt(claimRe.exec(__x));
      })(s.trim());
  if (x !== undefined) {
    return Belt_Array.map(Caml_option.valFromOption(x), (function (x) {
...
function makeClaim(l) {
  var xs = parseLine(l);
  return make(Caml_format.int_of_string(Belt_Array.getExn(xs, 1)), Caml_format.int_of_string(Belt_Array.getExn(xs, 2)), Caml_format.int_of_string(Belt_Array.getExn(xs, 3)), Caml_format.int_of_string(Belt_Array.getExn(xs, 4)), Caml_format.int_of_string(Belt_Array.getExn(xs, 5)));
}
...
function getClaimIdsFromArea(t, c) {
  var cids = Belt_Array.concatMany(Belt_Array.reduce(Belt_Array.range(c.x, (c.x + c.w | 0) - 1 | 0), [], (function (accX, x) {
              var __x = Belt_Array.reduce(Belt_Array.range(c.y, (c.y + c.h | 0) - 1 | 0), [], (function (accY, y) {
              ...
              return Belt_Array.concat(accX, __x);
            })));
...
function countNonOverlapClaim(t, xs) {
  var reducer = function (extra, extra$1) {
    var cid = getClaimIdsFromArea(t, extra$1);
    if (cid !== undefined) {
      return Belt_Array.concat(extra, [Caml_option.valFromOption(cid)]);
    } else {
      return extra;
    }
  };
