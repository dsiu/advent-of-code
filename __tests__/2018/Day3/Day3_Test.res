...
describe("2018 Day3", () => {
  describe("Part1", () => {
    ...
    test("fabric add claim (demo case)", () => {
      ...
      let test_fab =
        allClaims->Belt.Array.reduce(test_fab, (acc, i) => {
          acc->Fabric.addClaim(i)
        });

      // test_fab->Fabric.dump
      let gp = test_fab->Fabric.getPoint;
      let countOverlapTwoMore =
        test_fab->Fabric.countOverlap(_, Fabric.twoOrMore);
      let one = Some([1]);
      let two = Some([2]);
      let three = Some([3]);
