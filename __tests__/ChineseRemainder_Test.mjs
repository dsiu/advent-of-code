// Generated by ReScript, PLEASE EDIT WITH CARE

import * as Jest from "@glennsl/rescript-jest/src/jest.mjs";
import * as Jest2$AdventOfCode from "../interop/Jest2.mjs";
import * as ChineseRemainder$AdventOfCode from "../src/ChineseRemainder.mjs";

Jest.describe("ChineseRemainder", (function (param) {
        var int_tests = [
          [
            ChineseRemainder$AdventOfCode.crt([
                  3,
                  4
                ], [
                  2,
                  5
                ]),
            9
          ],
          [
            ChineseRemainder$AdventOfCode.crt([
                  2,
                  4
                ], [
                  4,
                  5
                ]),
            14
          ],
          [
            ChineseRemainder$AdventOfCode.crt([
                  2000,
                  300
                ], [
                  2,
                  33
                ]),
            36
          ],
          [
            ChineseRemainder$AdventOfCode.crt([
                  2,
                  12,
                  22
                ], [
                  13,
                  3,
                  2
                ]),
            54
          ],
          [
            ChineseRemainder$AdventOfCode.crt([
                  44,
                  12,
                  10
                ], [
                  2,
                  5,
                  3
                ]),
            22
          ]
        ];
        Jest.test("", (function (param) {
                return Jest.Expect.toEqual(Jest.Expect.expect(true), true);
              }));
        Jest2$AdventOfCode.testEach2("int", int_tests, (function (result, expected) {
                return Jest.Expect.toEqual(Jest.Expect.expect(result), expected);
              }));
        var bigint_tests = [
          [
            ChineseRemainder$AdventOfCode.crtBigInt([
                  BigInt(3),
                  BigInt(4)
                ], [
                  BigInt(2),
                  BigInt(5)
                ]),
            BigInt(9)
          ],
          [
            ChineseRemainder$AdventOfCode.crtBigInt([
                  BigInt("507483274265132509471575639764027"),
                  BigInt("27723967616827289286920296659419136")
                ], [
                  BigInt("269916455047188404153874847098609926219"),
                  BigInt("170141183460469231731687303715884105728")
                ]),
            BigInt("26140225850797799686267310622940276042186008390341917470797088078891078123520")
          ]
        ];
        Jest2$AdventOfCode.testEach2("big int", bigint_tests, (function (result, expected) {
                return Jest.Expect.toEqual(Jest.Expect.expect(result.toString()), expected.toString());
              }));
      }));

export {
  
}
/*  Not a pure module */
